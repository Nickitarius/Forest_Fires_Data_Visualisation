...
def create_map_loc_trace():
    ...
    df_loc = geodata_utils.load_geo_from_geojson(
        "geodata/localities_Irk_obl.geojson").drop(columns=['poly', 'address'])
...
def create_map_rail_trace():
    ...
    df_rail = geodata_utils.load_geo_from_geojson(
        "geodata/zhd_roads.geojson").drop(columns=['geom:1'])
    lats, lons = geodata_utils.get_coords_linestring(df_rail)
    return px.line_mapbox(lat=lats,
                          lon=lons,
